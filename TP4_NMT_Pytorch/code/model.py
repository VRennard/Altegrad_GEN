...
class seq2seqAtt(nn.Module):
    ...
    def forward(self,target_h,source_hs):
        ...
        concat_output =nn.Tanh()(self.ff_concat (torch.cat(( source_hs,target_h_rep), 2)) ) 
...
class Decoder(nn.Module):
    ...
    def forward(self, input, source_context, h):
        ...
        embedded = self.embedding(input)
        h_s, h = self.rnn(embedded)
        
        
        concat=nn.Tanh()(self.ff_concat (torch.cat((h_s, source_context), 2)) ) 

        prediction=self.predict(concat)
