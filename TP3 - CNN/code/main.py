...
def cnn_branch(n_filters,k_size,d_rate,my_input):
    U = Conv1D(filters = n_filters, kernel_size = k_size, activation = 'tanh')(my_input)
    V = GlobalMaxPooling1D()(U)
    return Dropout(d_rate)(V)
...
doc_wv = Embedding(max(word_to_index.values())+1, d)(doc_ints)### fill the gap (add an Embedding layer) https://keras.io/layers/embeddings/ ###
...
concat = Concatenate(axis=1)(branch_outputs) # branch output combination

preds = Dense(1,activation='sigmoid')(concat) ### fill the gap (add a Dense layer) https://keras.io/layers/core/ ###
...
get_doc_embedding = K.function([model.layers[0].input],[model.layers[9].output])
...
saliency_input = model.trainable_weights[0]     ### fill the gap (extract the rows of the embedding matrix from the model) ###
...
matrix = compute_gradients([my_review,0])[0]
